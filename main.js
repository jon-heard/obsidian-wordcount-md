...
class WordCountMd extends obsidian.Plugin
{
	onload()
	{
		BetterWordCount.statusBar = this.addStatusBarItem();
	...
	{
		const words = text.match(/[^\s/]*([a-zA-Z0-9])[^\s/]*/g).length;
		const chars = text.length;
		BetterWordCount.statusBar.setText(`${words} words ${chars} characters`);
	}
}
